...
func TestFullVPNProtocol(t *testing.T) {
	...
	// Step 2: Load client config from server.yaml
	t.Log("Loading client configuration...")
	serverConfig, err := LoadConfig("../../server.yaml")
	if err != nil {
		serverCancel()
		t.Fatalf("Failed to load server config: %v", err)
	}
	
	if len(serverConfig.Clients) == 0 {
		serverCancel()
		t.Fatalf("No clients configured in server.yaml")
	}
	
	// Use first client from config
	testClient := serverConfig.Clients[0]
	
	// Step 3: Connect client with pre-shared key
	t.Log("Connecting VPN client...")
	client, err := client.NewClient(serverAddr, testClient.ID, testClient.Key)
	if err != nil {
		serverCancel()
