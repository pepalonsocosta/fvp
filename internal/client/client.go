...
// NewClient creates a new VPN client with pre-shared key
func NewClient(serverAddr string, clientID uint8, keyHex string) (*Client, error) {
	// Decode hex key
...
func (c *Client) waitForAuthResponse() error {
	c.udpConn.SetReadDeadline(time.Now().Add(config.DefaultClientUDPTimeout))
	...
	// Extract assigned IP from server response
	// Server validates our pre-shared key and sends back the IP
...
func (c *Client) handleServerPackets() {
	...
	for {
		...
		case <-c.stopChan:
			...
			return
		default:
			c.udpConn.SetReadDeadline(time.Now().Add(config.DefaultUDPReadTimeout))
...
func (c *Client) handleTUNPackets() {
	...
	for {
		...
		default:
			...
			if err != nil {
				time.Sleep(config.DefaultTUNReadDelay)
				continue
			}
