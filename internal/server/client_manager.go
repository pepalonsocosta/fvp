...
func (cm *ClientManager) AddClient(key []byte, address string) (*Client, error) {
	...
	if len(cm.clients) >= config.MaxClients {
...
func (cm *ClientManager) startTimeoutChecker() {
	ticker := time.NewTicker(config.DefaultTimeoutCheckInterval)
	defer ticker.Stop()
...
func (cm *ClientManager) assignNextIP() string {
	for i := config.VPNClientIPStart; i <= config.VPNClientIPEnd; i++ {
		ip := fmt.Sprintf("%s.%d", config.VPNNetworkBase, i)
...
func (cm *ClientManager) determineClient(packetData []byte) (uint8, error) {
	...
	if destinationIP == config.VPNServerIP {
